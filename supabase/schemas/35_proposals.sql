-- Proposals: how a lead becomes a customer. Staff price a deal from the price book, send the
-- proposal as a link, and the person it is addressed to accepts it online — signed in with
-- that email — which is what turns the lead into a tenant: accepting creates the owner
-- membership, marks the customer won with the proposal's value, and records who accepted,
-- when and from where. That record is the signature.
--
--   price_book_items   what can be sold — the product's plans and add-ons with a unit price.
--                      The table is framework; the rows are product data, entered by staff.
--   proposals          one deal, addressed to an email, with a status and a token link. A
--                      draft is edited freely; sending freezes it and starts the clock.
--   proposal_lines     the lines on a proposal, from the price book or bespoke, with their
--                      quantities. The proposal's totals are kept by trigger.
--
-- Amounts are in the site's currency (src/config/site.ts), whole units with cents, like
-- customers.annual_value. `kind` says how a line recurs, which is what makes the annual
-- figure: annual lines count once, monthly ones twelve times, one-off ones not at all.
--
-- Access: every staff tier reads all of it; drafting, sending and withdrawing need
-- platform_can_manage_customers(). The recipient — usually not a user yet — reads through
-- get_proposal(token) and answers through accept_proposal() / decline_proposal(), the same
-- email-bound shape as invitations.
