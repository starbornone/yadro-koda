...
| `schemas/30_crm.sql`           | Staff-only CRM over organisations: `customers` (one per organisation by trigger; `customer_stage`, owner, source, `details` jsonb of product-defined fields), `contacts` (linked to a member by trigger on join), `activities` (stage changes, joins and website enquiries written by the database), `tasks`; `platform_can_manage_customers()`; `customer_stage_counts` view; `create_lead()` RPC; `submit_enquiry()`, the anonymous lead form's RPC, with `enquiry_attempts` for its rate limit |
