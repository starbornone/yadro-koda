...
| `schemas/20_organisations.sql` | `organisations`, `memberships` (`org_role`, `expires_at`), `platform_members` (`platform_role` superadmin/admin/support, tier-managed, last superadmin protected), RLS helpers incl. `platform_can_manage_org()` and `shares_org_with()`, `create_organisation()` RPC        |
| `schemas/30_crm.sql`           | Staff-only CRM over organisations: `customers` (one per organisation by trigger; `customer_stage`, owner, source), `contacts`, `activities` (stage changes logged by trigger), `tasks`; `platform_can_manage_customers()`; `customer_stage_counts` view; `create_lead()` RPC |
