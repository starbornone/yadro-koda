...
| `schemas/35_proposals.sql`     | The deal: `price_book_items` (`price_book_kind` one_off/monthly/annual), `proposals` (`proposal_status` draft → sent → accepted/declined/withdrawn; email-bound token link; totals kept by trigger; one sent per organisation) and `proposal_lines`; `send_proposal()` / `withdraw_proposal()` for staff, `get_proposal()` (works signed out) / `accept_proposal()` / `decline_proposal()` for the recipient — accepting creates the owner membership and wins the customer                                                                                                                                                                        |
| `schemas/30_crm.sql`           | Staff-only CRM over organisations: `customers` (one per organisation by trigger; `customer_stage`, owner, source, `details` jsonb of product-defined fields, plan, annual value, expected close / renewal dates, outcome reason; `stage_changed_at` and `won_at` kept by trigger), `contacts` (linked to a member by trigger on join), `activities` (stage changes, joins and website enquiries written by the database), `tasks`; `platform_can_manage_customers()`; `customer_stage_summary` view (count and value by stage); `create_lead()` RPC; `submit_enquiry()`, the anonymous lead form's RPC, with `enquiry_attempts` for its rate limit |
